...
function switchNaughtyMonth(switchMonths)
        ...
        naughty.notify({
                text = string.format('<span font_desc="%s">%s</span>', "Envy Code R For Powerline", displayMonth(calendar[1], calendar[2], 2)),
                timeout = 0,
                hover_timeout = 0.5,
...
function addCalendarToWidget(mywidget, custom_current_day_format)
  ...
  mywidget:connect_signal('mouse::enter', function ()
        ...
        calendar = { month, year,
        naughty.notify({
                text = string.format('<span font_desc="%s">%s</span>', "Envy Code R For Powerline", displayMonth(month, year, 2)),
                timeout = 0,
                hover_timeout = 0.5,
